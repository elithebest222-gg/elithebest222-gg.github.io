...
const installProviders = ["adinplay", "cpmstar", "local"];

const videoAdPriorities = ["adinplay", "cpmstar"];

const bannerAdPriorities = ["adinplay", "cpmstar", "local"];
...
Object.keys(window.bannerDimensions).forEach((key) => {
  window.bannerDimensions[key].ratioBoostStops.sort(
    (a, b) => a.ratio - b.ratio
  );
});
...
window.SDK = {
  ...
  _startTrackingShowtime(adTag) {
    ...
    if (!this._bannerVisibleSince[adTag]) {
      ...
      if (DEBUG_MODE)
        console.log(`[sdk.js] Started tracking showtime for ${adTag}`);
    }
  },
  ...
  _stopTrackingShowtime(adTag) {
    if (this._bannerVisibleSince[adTag]) {
      ...
      this._bannerShowtime[adTag] =
        (this._bannerShowtime[adTag] || 0) + visibleDuration;
      delete this._bannerVisibleSince[adTag];
      if (DEBUG_MODE)
        console.log(
          `[sdk.js] Stopped tracking showtime for ${adTag}. Total showtime: ${Math.floor(
            this._bannerShowtime[adTag] / 1000
          )}s`
        );
    }
  },
  ...
  async _originalSetBanner(bannerType, bannerPosition) {
    ...
    if (!adTag || !dims) {
      if (DEBUG_MODE)
        console.log(`[sdk.js] SetBanner: Invalid banner type ${bannerType}`);
      return;
    }

    if (IS_MOBILE && !dims.enableForMobile) {
      if (DEBUG_MODE)
        console.log(`[sdk.js] SetBanner: Banner ${adTag} disabled on mobile`);
      return;
    }

    // Get position name for logging
    const posName =
      POSITION_NAMES[bannerPosition] || `Position${bannerPosition}`;
    ...
    if (existingInstance) {
      ...
      if (
        this._occupiedPositions[newPosKey] &&
      ...
      if (shouldRefresh) {
        ...
        this._startTrackingShowtime(adTag);
        this._displayBannerWithProviders(
          bannerType,
          adTag,
  ...
  async _displayBannerWithProviders(bannerType, adTag, container, now) {
    ...
    const posName = bannerInstance
      ? POSITION_NAMES[bannerInstance.position] ||
        `Position${bannerInstance.position}`
      : "Unknown";
    ...
    const tryNextProvider = async () => {
      if (providerIndex >= priorities.length) {
        if (DEBUG_MODE)
          console.log(
            `[sdk.js] All providers failed for banner ${adTag} at ${posName}`
          );
        return false;
      }
      ...
      if (!provider) {
        if (DEBUG_MODE)
          console.log(
            `[sdk.js] Provider ${providerName} not available for banner ${adTag}`
          );
        providerIndex++;
      ...
      if (DEBUG_MODE)
        console.log(
          `[sdk.js] Trying to show banner ${adTag} at ${posName} with provider: ${providerName}`
        );
      ...
      try {
        ...
        if (success) {
          if (DEBUG_MODE)
            console.log(
              `[sdk.js] Successfully showing banner ${adTag} at ${posName} with ${providerName}`
            );
          ...
          if (DEBUG_MODE)
            console.log(
              `[sdk.js] Showing banner ${adTag} at ${posName} with ${providerName} FAILED, trying next...`
            );
          providerIndex++;
          return tryNextProvider();
        }
        ...
        if (DEBUG_MODE)
          console.log(
            `[sdk.js] Error showing banner ${adTag} with ${providerName}:`,
            error.message || error
          );
        providerIndex++;
        return tryNextProvider();
...
window.SDK.SetBanner = function (bannerType, bannerPosition) {
  ...
  if (!window.SDK._isInitialized && bannerPosition !== 0) {
    if (DEBUG_MODE)
      console.log(
        `[sdk.js] SDK not initialized yet, queueing banner ${
          window.bannerMapping[bannerType] || bannerType
        } for position ${bannerPosition}`
      );

    // Check if we already have this banner type in queue
    const existingIndex = window.SDK._pendingBannerQueue.findIndex(
      (item) => item.bannerType === bannerType
    );
    if (existingIndex >= 0) {
      // Update existing entry with new position
      window.SDK._pendingBannerQueue[existingIndex].bannerPosition =
        bannerPosition;
    } else {
      // Add new entry
  ...
  if (bannerPosition === 0) {
    ...
    if (window.SDK._isInitialized) {
      window.SDK._originalSetBanner.call(
        window.SDK,
        bannerType,
        bannerPosition
      );
    ...
    window.SDK._debouncedSetBannerPerType[bannerType](
      bannerType,
      bannerPosition
    );
  }
  ...
  Object.keys(window.SDK._bannerAds).forEach((adTag) => {
    ...
    if (window.SDK._isBannerVisible(banner)) {
      ...
      if (currentShowtime >= MIN_REFRESH_INTERVAL) {
        if (DEBUG_MODE)
          console.log(
            `[sdk.js] Banner ${adTag} reached ${Math.floor(
              currentShowtime / 1000
            )}s showtime, refreshing...`
          );

        const bannerType = Object.keys(window.bannerMapping).find(
          (key) => window.bannerMapping[key] === adTag
        );
        if (bannerType !== undefined) {
          // Reset showtime before refresh
          window.SDK._resetShowtime(adTag);
          window.SDK._startTrackingShowtime(adTag);
          window.SDK._displayBannerWithProviders(
            parseInt(bannerType),
            adTag,
            banner.container,
            now
...
window.SDK._processQueuedBanners = function () {
  if (DEBUG_MODE && window.SDK._pendingBannerQueue.length > 0) {
    console.log(
      `[sdk.js] Processing ${window.SDK._pendingBannerQueue.length} queued banner(s)`
    );
  }
  ...
  queue.forEach(({ bannerType, bannerPosition }) => {
    if (DEBUG_MODE) {
      ...
      console.log(
        `[sdk.js] Processing queued banner: ${adTag} at position ${bannerPosition}`
      );
    }
